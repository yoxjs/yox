...
(function (global, factory) {
  ...
  AnonymousFunction.prototype.toString = function (tabSize) {
      var ref = this;
          ...
          var currentTabSize = tabSize || 0, nextTabSize = currentTabSize + 1, currentIndentSize = repeat(INDENT, currentTabSize), nextIndentSize = repeat(INDENT, nextTabSize), tuple = args ? toTuple(EMPTY_STRING, EMPTY_STRING, COMMA, FALSE$1, args).toString(currentTabSize) : EMPTY_STRING, code = [];
      if (body) {
          ...
          if (!falsy$2(callNode.args)) {
              ...
              push(args, toMap({
                  args: toAnonymousFunction([
                      ...
                      toRaw(ARG_MAGIC_VAR_DATA) ], UNDEFINED$1, toList(callNode.args.map(generateExpressionArg)))
              }));
          }
          else {
              // runtime
  ...
  nodeGenerator[EACH] = function (node) {
      ...
      var renderChildren = toAnonymousFunction(args, UNDEFINED$1, generateNodesToList(node.children));
      if (index) {
          pop(magicVariables);
      }
      pop(eachStack);
  ...
  nodeGenerator[PARTIAL] = function (node) {
      return toCall(DEFINE_PARTIAL, [
          ...
          toAnonymousFunction([
              toRaw(ARG_MAGIC_VAR_KEYPATH),
              toRaw(ARG_CHILDREN),
              toRaw(ARG_COMPONENTS) ], UNDEFINED$1, generateNodesToList(node.children))
      ]);
  };
  ...
  function render(instance, template, scope, filters, globalFilters, partials, globalPartials, directives, globalDirectives, transitions, globalTransitions) {
      ...
      dependencies = {}, lookupValue = function (stack, index, key) {
          var context = stack[index], keypath = join(context.keypath, key), result = get(context.scope, keypath);
          if (result) {
      ...
      renderPartial = function (name, keypath, children, components) {
          ...
          if (array$1(value)) {
              ...
              for (var key in value) {
                  ...
                  result.push(renderChildren(currentKeypath, UNDEFINED$1, value[key], key));
              }
          }
          ...
          var scope = ref.scope;
          var name = tokens ? tokens.join(RAW_DOT) : EMPTY_STRING, currentKeypath = join(keypath, name);
          var result;
          if (tokens) {
