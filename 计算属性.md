...
关闭缓存前面已经提过，这种方式比较简单粗暴。

下面我们采用第二种方式，把 `getter` 函数进行改写：

```javascript
{
    allChecked: {
        get: function () {
            var list = this.get('list');                for (var i = 0, len = list.length; i < len; i++) {                    if (!list[i].checked) {                        return false;                    }                }                return len > 0;
        }
    }
}
```
